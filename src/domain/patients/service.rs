...
mod tests {
    ...
    use crate::infrastructure::postgres_repository_impl::{create_tables::create_tables, patients::PostgresPatientsRepository};

    async fn setup_service(pool: sqlx::PgPool) -> PatientsService {
        create_tables(&pool, true).await.unwrap();
        PatientsService::new(Box::new(PostgresPatientsRepository::new(pool)))
    }
